import { NextResponse } from 'next/server';
import fs from 'fs';
import path from 'path';

export async function GET() {
  try {
    const dataPath = path.join(process.cwd(), 'data', 'feedback.json');
    if (!fs.existsSync(dataPath)) return NextResponse.json([]);
    const fileContents = fs.readFileSync(dataPath, 'utf8');
    return NextResponse.json(JSON.parse(fileContents));
  } catch (error) {
    return NextResponse.json({ error: 'Failed to read feedback' }, { status: 500 });
  }
}

export async function POST(request: Request) {
  try {
    const data = await request.json();
    const dataPath = path.join(process.cwd(), 'data', 'feedback.json');
    let feedback = [];
    if (fs.existsSync(dataPath)) {
      feedback = JSON.parse(fs.readFileSync(dataPath, 'utf8'));
    }
    feedback = [data, ...feedback];
    fs.writeFileSync(dataPath, JSON.stringify(feedback, null, 2), 'utf8');
    return NextResponse.json({ message: 'Success' });
  } catch (error) {
    return NextResponse.json({ error: 'Failed to save feedback' }, { status: 500 });
  }
}

export async function PUT(request: Request) {
  try {
    const data = await request.json(); // Expected to be an array
    const dataPath = path.join(process.cwd(), 'data', 'feedback.json');
    fs.writeFileSync(dataPath, JSON.stringify(data, null, 2), 'utf8');
    return NextResponse.json({ message: 'Success' });
  } catch (error) {
    return NextResponse.json({ error: 'Failed to update feedback' }, { status: 500 });
  }
}
